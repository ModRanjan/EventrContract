...
pub contract Eventr: NonFungibleToken {
    ...
    pub resource Event {
        ...
        pub fun batchMintToken(categoryId: UInt64, quantity: UInt64): @Collection {
    ...
    pub resource interface NFTMinterPublic {
        // pub fun mint(eventID: UInt64, categoryID: UInt64, _price: UFix64, _payment: @FlowToken.Vault, _collection: &&AnyResource{CollectionPublic}, _flowTokenVault: Capability<&FlowToken.Vault{FungibleToken.Receiver}>)
      
        pub fun mint(eventID: UInt64, categoryID: UInt64, _price: UFix64, _collection: &AnyResource{CollectionPublic})
    ...
    pub resource Admin: NFTMinterPublic {
        ...
        // pub fun mint(address: Address,eventID: UInt64, categoryID: UInt64, _price: UFix64, _collection: &Eventr.Collection): @Eventr.NFT? {
        pub fun mint(eventID: UInt64, categoryID: UInt64, _price: UFix64, _collection: &AnyResource{CollectionPublic}) {      
            pre {
                Eventr.passCategoryData[categoryID] != nil : "Cannot find any Category on given categoryID"
                _price == Eventr.passCategoryData[categoryID]?.price : "NFT price and vault balance should be same"
            }
            
            let userCapability = _collection
            let eventRef = self.borrowEvent(eventID: eventID)

            let newToken <- eventRef.mintToken(categoryID: categoryID)

            // deposit the NFT in the receivers collection
            userCapability.deposit(token: <- newToken)
           
            // return <- eventRef.mintToken(eventID: eventID, categoryID: categoryID)
        }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection { 
        ...
        // They can use this to read its eventId,passId,categoryId
        // or any of the eventData or Pass data associated with it by
        // getting the eventID or passID and reading those fields from
        // the smart contract.
        //
        // Parameters: id: The ID of the NFT to get the reference for
        //
    ...
    pub fun createAdmin(): @Eventr.Admin {
        return <-create Eventr.Admin()
    }
    ...
    pub fun getAllEvents(): [Eventr.EventData] {
      let temp = Eventr.eventDatas.values
    ...
    pub fun getPassCategoryData(categoryID: UInt64): Eventr.PassCategory? {
        if Eventr.passCategoryData[categoryID] != nil {
            return Eventr.passCategoryData[categoryID]
        }
