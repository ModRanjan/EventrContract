...
pub contract Eventr1: NonFungibleToken {
    ...
    pub resource interface NFTMinterPublic {
        pub fun testMintToken(eventID: UInt64, categoryID: UInt64, price: UFix64, collection: &AnyResource{CollectionPublic})

        pub fun testBatchMintToken(eventID: UInt64, categoryID: UInt64, quantity: UInt32, price: UFix64, collection: &AnyResource{CollectionPublic})
        ...
        // pub fun batchMintToken(eventID: UInt64, categoryID: UInt64, quantity: UInt32, collection: &AnyResource{CollectionPublic}, flowTokenVault: Capability<&FlowToken.Vault{FungibleToken.Receiver}>, buyerFlowTokenVault: &FlowToken.Vault)
    }
    ...
    pub resource Admin: NFTMinterPublic {
        ...
        pub fun testMintToken(eventID: UInt64, categoryID: UInt64, price: UFix64, collection: &AnyResource{CollectionPublic}) {      
            ...
            pre {
                ...
                price == Eventr1.passCategoryData[categoryID]?.price! : "Not enough tokens to buy the NFT!"
            }
        ...
        pub fun testBatchMintToken(eventID: UInt64, categoryID: UInt64, quantity: UInt32, price: UFix64, collection: &AnyResource{CollectionPublic}) {
            pre {
                Eventr1.events[eventID] != nil : "Cannot find any Event on given eventID"
                Eventr1.passCategoryData[categoryID] != nil : "Cannot find any Category on given categoryID"
                price == (UFix64(quantity) * Eventr1.passCategoryData[categoryID]?.price!) : "Not enough tokens to buy the NFT!"
            }
            let eventRef = self.borrowEvent(eventID: eventID)
           
            let newTokenCollection <- eventRef.batchMintToken(categoryID: categoryID, quantity: quantity)

            // deposit the NFT in the receivers collection
            collection.batchDeposit(tokens: <- newTokenCollection)
        }
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection { 
        ...
        // borrowNFT Returns a borrowed reference to a Tokems in the Collection
